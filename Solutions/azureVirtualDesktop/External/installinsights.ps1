...
param(
    ...
    [Parameter(mandatory = $true)][string]$rgLaw,
...
function Install-VMssExtension {
    ...
    $scalesetObject = Get-AzMVMSS -VMScaleSetName $VMScaleSetName -ResourceGroupName $VMScaleSetResourceGroupName
...
$key = $law | Get-AzOperationalInsightsWorkspaceSharedKey
$WorkspaceKey = $key.PrimarySharedKey
$WorkspaceRegion = $law.Location
